...
import { FormControl, Grid, IconButton, FormControlLabel } from "@material-ui/core";
...
const ScheduleModal = ({ open, onClose, scheduleId, contactId, cleanContact, reload }) => {
	...
	useEffect(() => {
		if (contactId && contacts.length > 0) {
			...
			setSchedule(prevState => ({
				...prevState,
				contactId: Array.isArray(contactId) ? contactId : [contactId],
			}))
		}
	}, [contactId, contacts]);
		...
		if (open) {
			try {
				(async () => {
					...
					let customList = contactList.map(c => ({ id: c.id, name: c.name })).filter(contact => contact.id !== "");
					...
					if (contactId) {
						const contact = customList.find(c => c.id === contactId);
	...
	const handleSaveSchedule = async values => {
		...
		try {
			if (scheduleId) {
				await Promise.all(
					schedule.contactId.map(async (contactId) => {
						const contactScheduleData = { ...scheduleData, contactId };
						await api.put(`/schedules/${scheduleId}`, contactScheduleData);
				...
				await Promise.all(
					schedule.contactId.map(async (contactId) => {
						const contactScheduleData = { ...scheduleData, contactId };
						const { data } = await api.post("/schedules", contactScheduleData);
									...
									<FormControl variant="outlined" fullWidth>
										...
										<Autocomplete
											...
											renderOption={(option, { selected }) => (
												<>
													<Checkbox checked={schedule.contactId.includes(option.id)} style={{ marginRight: 8 }} />
													{option.name}
												</>
											)}
